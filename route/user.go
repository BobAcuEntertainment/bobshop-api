...
func init() {
	handlers = append(handlers, func(m *middleware, r *gin.Engine) {
		...
		v1cms.GET("/roles", s.BearerAuth(enum.PermissionUserView), s.v1cms_ListRole())
	})
}
...
func (s user) v1cms_Create() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, id := range data.RoleIds {
			role, err := s.store.Db.Role.FindOneById(c.Request.Context(), id)
...
// @Param user_id path string true "user_id"
// @Success 200 {object} db.UserCmsDto
// @Failure 401,403,404,500 {object} ecode.Error
...
func (s user) v1cms_ListRole() gin.HandlerFunc {
	return func(c *gin.Context) {
		session := s.Session(c)
