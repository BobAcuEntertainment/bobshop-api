...
func (s storage) v1_Download(kind string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if file, err := s.store.Db.Bucket.OpenDownloadStreamByName(fmt.Sprintf("/%v/%v", kind, filename)); err != nil {
			...
			c.DataFromReader(http.StatusOK, int64(info.Length), info.Metadata.Lookup("Content-Type").StringValue(), file, map[string]string{"Cache-Control": info.Metadata.Lookup("Cache-Control").StringValue()})
		}
	}
}
...
func (s storage) v1_Upload(kind string, exts []string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		for _, file := range files {
			if !slices.ContainsFunc(exts, func(e string) bool { return strings.HasPrefix(file.Header.Get("Content-Type"), e) }) {
