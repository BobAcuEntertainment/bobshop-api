...
/** Strategy */
func (s Store) RunStrategy(ctx context.Context, domain *db.CoinConfigDomain) error {
	var (
		strategy, _ = s.Db.Strategy.FindOneById(ctx, gopkg.Value(domain.StrategyId))
	...
	gopkg.LoopFunc(gopkg.Value(domain.TraderIds), func(id string) {
		trader, _ := s.Db.Trader.FindOneById(ctx, id)
	...
	return s.Rdb.Publish(ctx, "run_strategy", strategyDto.Bytes())
}

func (s Store) DelStrategy(ctx context.Context, id string) error {
	return s.Rdb.Publish(ctx, "del_strategy", []byte(id))
}
