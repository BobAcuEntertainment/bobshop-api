...
func New() *Db {
	...
	db := &Db{
		...
		CoinConfig: newCoinConfig(ctx, mongodb.Collection("coin_config")),
		CoinOrder:  newCoinOrder(ctx, mongodb.Collection("coin_order")),
		Role:       newRole(ctx, mongodb.Collection("role")),
		Strategy:   newStrategy(ctx, mongodb.Collection("strategy")),
		Tenant:     newTenant(ctx, mongodb.Collection("tenant")),
		Trader:     newTrader(ctx, mongodb.Collection("trader")),
